...
var level2 = {
	preload: function() {        // se cargan todas la imágenes en el preload
		...
		game.load.image('fondol1', 'imagenes/fondo1.6.jpg');//la imagen del fondo del juego
	...
	create: function() {           // crea los actores y muestra las imágenes
        ...
        this.puntaje_texto = this.add.text(10, 20, 'puntaje: '+puntuacion2, { fontSize: '22px', fill: "#1517E9" });// Puntajes
		this.level = this.add.text(game.width-50, 20, 'lvl 2 ', { fontSize: '22px', fill: "#1517E9" });
	...
	update: function(){                // ejecuta de manera reiterativa
		...
		moneda.position.x-= 1; //le agrega un efecto de movimiento a las balas de derecha a izquierda
		bala.position.x-= 1; //le agrega un efecto de movimiento a las balas de derecha a izquierda
		...
		if(Izquierda.isDown){          //  una función if en caso de que el jugador pulse izqu
			...
			bala.position.x += 0.5; // se realentiza la velocidad de la bala para que de un efecto de que el personaje está retrocediendo
		...
		this.colisiones (); //correRa una función que analiza si existe colisión entre el personaje y los objetos y midifica el puntaje 
	...
	colisiones: function(){
     ...
     if(difx_bala< 25 && dify_bala< 35){this.tocar_bala(bala);         
		puntuacion2 -= 2; //disminuye el puntaje en 2 en caso de que el personaje toque la bala
		this.puntaje_texto.setText('puntaje: ' + puntuacion2);//Actualiza el puntaje en caso de que los objetos se toquen 
		}
     ...
     if(difx_moneda< 25 && dify_moneda< 65){this.tocar_moneda(moneda);         
		puntuacion2 += 1; //aumenta elpuntaje en uno en caso de que el personaje toque la moneda
		this.puntaje_texto.setText('puntaje: ' + puntuacion2);//Actualiza el puntaje en caso de que los objetos se toquen 
		} 
         //Considicion de paso al siguiente nivel
        if(puntuacion2>9){
            game.state.start("level3");   // en caso de que cumpla con un puntaje mayor a 10 llega al lvl 2 (CAMBIO DE ESCENARIO)
        }
        if(puntuacion2<0){
            game.state.start("lose");   // si su puntaje llega a menos de 0 vuelve a de perdedor (CAMBIO DE ESCENARIO)
        }
	},
        ...
        this.puntaje_texto.setText('puntaje: ' + puntuacion2);//Actualiza el puntaje      
    },
